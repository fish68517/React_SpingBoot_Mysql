package com.graduation.controller;

import com.graduation.repository.ArtworkRepository;
import com.graduation.repository.CommentRepository;
import com.graduation.repository.UserRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import java.util.HashMap;
import java.util.Map;

@RestController
@RequestMapping("/api/admin/dashboard")
@CrossOrigin(origins = "*")
public class DashboardController {

    @Autowired
    private UserRepository userRepository;

    @Autowired
    private ArtworkRepository artworkRepository;

    // 假设你有一个 CommentRepository
    // @Autowired
    private CommentRepository commentRepository;

    @GetMapping("/stats")
    public Map<String, Object> getDashboardStats() {
        Map<String, Object> stats = new HashMap<>();

        // 1. 基础计数
        long totalUsers = userRepository.count();
        long totalArtworks = artworkRepository.count();
        // long totalComments = commentRepository.count();
        long totalComments = 1250; // 模拟数据，等你有了 CommentRepository 后替换

        // 2. 待处理事项计数
        // 假设 artworkRepository 有一个 countByStatus 方法
        // long pendingArtworks = artworkRepository.countByStatus(0);
        long pendingArtworks = 5; // 模拟

        stats.put("totalUsers", totalUsers);
        stats.put("totalArtworks", totalArtworks);
        stats.put("totalComments", totalComments);
        stats.put("pendingArtworks", pendingArtworks);

        // 3. 模拟图表数据 (例如最近7天的访问量)
        int[] weeklyVisits = {120, 132, 101, 134, 90, 230, 210};
        stats.put("weeklyVisits", weeklyVisits);

        return stats;
    }
}